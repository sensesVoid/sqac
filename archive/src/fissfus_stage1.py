...
class PQVectorQuantizer(nn.Module):
    """Product Quantization style sub-codebook quantizer."""
...
class VQVAE(nn.Module):
    def __init__(self, chunk_size: int = 8, latent_dim: int = 256, codebook_size: int = 1024, 
    ...
    def forward(self, x: torch.Tensor, update_ema: bool = True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        z = self.encoder(x)
...
def semantic_dedup(chunks: List[np.ndarray], encoder: nn.Module, threshold: float = 0.85, device: str = "cpu") -> List[np.ndarray]:
    """Remove semantically similar chunks using VSA cosine similarity."""
    if len(chunks) <= 1:
        return chunks
    
    encoder.eval()
    with torch.no_grad():
        x = torch.tensor(np.stack(chunks), dtype=torch.float32).to(device)
    ...
    for i in range(len(chunks)):
        if not kept_z:
            ...
            kept_z.append(z_norm[i])
        else:
            sims = np.dot(kept_z, z_norm[i])
            if sims.max() < threshold:
                kept.append(chunks[i])
                kept_z.append(z_norm[i])
    return kept

def train_vqvae(text: str, chunk_size: int = 8, latent_dim: int = 256,
                codebook_size: int = 1024, num_subspaces: int = 4, epochs: int = 400,
                batch_size: int = 512, lr: float = 1e-3, device: str = "cpu",
                dedup_threshold: float = 0.0, entropy_weight: float = 0.0) -> Tuple[VQVAE, Stage1Result]:
    ...
    if dedup_threshold > 0:
        temp_encoder = CharEmbeddingEncoder(chunk_size, latent_dim).to(device)
        dataset_chunks = semantic_dedup(chunks, temp_encoder, dedup_threshold, device)
        print(f"  Dedup: {len(chunks)} -> {len(dataset_chunks)} chunks ({len(dataset_chunks)/len(chunks):.1%})")
    
    dataset = torch.tensor(np.stack(dataset_chunks), dtype=torch.float32)
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(dataset), batch_size):
            ...
            recon_logits, indices, embedding_loss, commitment_loss = model(x)
...
if __name__ == "__main__":
    ...
    print("\n--- Larger model + entropy reg ---")
    model, result = train_vqvae(text, chunk_size=8, latent_dim=128, codebook_size=1024, 
                                num_subspaces=8, epochs=200, batch_size=1024, lr=5e-4, 
                                dedup_threshold=0.0, entropy_weight=0.02)
